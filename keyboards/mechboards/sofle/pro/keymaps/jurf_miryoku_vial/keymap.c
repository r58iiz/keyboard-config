...
const char *tap_dance_string(uint16_t index) {
    switch (index) {
        case U_TD_BOOT:
            return "RST";
#define MIRYOKU_X(LAYER) \
    case U_TD_U_##LAYER: \
        return layer_string(U_##LAYER);
            MIRYOKU_LAYER_LIST
#undef MIRYOKU_X
        default:
            return "TD";
...
// [qmk2vial] Removed tap_dance_actions for Vial compatibility
...
// [qmk2vial] Removed get_tapping_term function for Vial compatibility
