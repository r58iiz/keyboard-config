...
__attribute__((weak)) const char *tap_dance_string(uint16_t index) {
    static char buf[4];
    snprintf(buf, sizeof(buf), "TD%u", index);
    return buf;
}

static const char *special_basic_key_string(uint16_t keycode) {
    switch (keycode) {
        case KC_ENT:
            return "RET";
        case KC_ESC:
...
__attribute__((weak)) const char *keycode_string(uint16_t keycode) {
    ...
    switch (keycode) {
        case 0 ... 56:
            const char *special = special_basic_key_string(keycode);
            if (special) {
                return special;
            }
            static char key[2];
            key[0] = pgm_read_byte(&basic_codes_to_name[keycode]);
            key[1] = '\0';
            return key;
        case KC_CAPS:
            keycode_str = "Caps\0";
        ...
        case QK_MOD_TAP ... QK_MOD_TAP_MAX:
            // keycode_str = "MT()\0"; break;
            return keycode_string(keycode & 0xFF);
        case QK_LAYER_TAP ... QK_LAYER_TAP_MAX:
            // keycode_str = "LT()\0"; break;
            return keycode_string(keycode & 0xFF);
        ...
        case QK_TAP_DANCE ... QK_TAP_DANCE_MAX:
            // keycode_str = "TD()\0"; break;
            return tap_dance_string(QK_TAP_DANCE_GET_INDEX(keycode));
